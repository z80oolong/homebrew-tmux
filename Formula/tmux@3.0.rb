...
index c29292b..380c530 100644
--- a/cfg.c
+++ b/cfg.c
@@ -73,6 +73,10 @@ start_cfg(void)
 	int		 flags = 0;
 	struct client	*c;
...
index ba7db3e..ce65cde 100644
--- a/options-table.c
+++ b/options-table.c
@@ -849,5 +849,37 @@ const struct options_table_entry options_table[] = {
...
+#endif
 	...
 	setlocale(LC_TIME, "");
 	tzset();
@@ -348,6 +357,21 @@ main(int argc, char **argv)
...
@@ -77,11 +154,91 @@ tty_acs_cmp(const void *key, const void *value)
 	ch = *(u_char *) key;
 	return (ch - entry->key);
 }
...
@@ -102,12 +259,31 @@ tty_acs_needed(struct tty *tty)
 	if (tty->flags & TTY_UTF8)
 		return (0);
 	return (1);
+#endif /* NO_USE_PANE_BORDER_ACS_ASCII */
...
@@ -123,4 +299,5 @@ tty_acs_get(struct tty *tty, u_char ch)
 	if (entry == NULL)
 		return (NULL);
 	return (entry->string);
+#endif /* NO_USE_PANE_BORDER_ACS_ASCII */
...
index 182edd7..9e8ab7e 100644
--- a/tty-term.c
+++ b/tty-term.c
@@ -546,6 +546,15 @@ tty_term_find(char *name, int fd, char **cause)
...
+#else
 	...
 	memset(term->acs, 0, sizeof term->acs);
 	if (tty_term_has(term, TTYC_ACSC))
@@ -554,6 +563,7 @@ tty_term_find(char *name, int fd, char **cause)
