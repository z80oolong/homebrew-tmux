...
index 8264d1b..d6451c8 100644
--- a/options-table.c
+++ b/options-table.c
@@ -1088,6 +1088,38 @@ const struct options_table_entry options_table[] = {
...
index 2992178..736cc23 100644
--- a/tmux.c
+++ b/tmux.c
@@ -327,20 +327,33 @@ main(int argc, char **argv)
 {
...
+#endif
 	...
 	setlocale(LC_TIME, "");
 	tzset();
@@ -353,7 +366,16 @@ main(int argc, char **argv)
...
+#endif
 	...
 	while ((opt = getopt(argc, argv, "2c:CDdf:lL:NqS:T:uUvV")) != -1) {
 		switch (opt) {
@@ -484,6 +506,19 @@ main(int argc, char **argv)
...
@@ -509,6 +544,13 @@ main(int argc, char **argv)
 	socket_path = path;
 	free(label);
