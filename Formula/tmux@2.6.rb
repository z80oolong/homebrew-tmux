...
index f5e973e..8508658 100644
--- a/options-table.c
+++ b/options-table.c
@@ -935,5 +935,28 @@ const struct options_table_entry options_table[] = {
 	  .default_num = 1
 	},
...
+#endif
 	...
 	setlocale(LC_TIME, "");
 	tzset();
@@ -328,6 +337,13 @@ main(int argc, char **argv)
...
index 9d026fa..23e6db5 100644
--- a/tty-term.c
+++ b/tty-term.c
@@ -510,6 +510,15 @@ tty_term_find(char *name, int fd, char **cause)
...
+#else
 	...
 	memset(term->acs, 0, sizeof term->acs);
 	if (tty_term_has(term, TTYC_ACSC))
@@ -518,6 +527,7 @@ tty_term_find(char *name, int fd, char **cause)
...
+#ifndef NO_USE_UTF8CJK
+// Prototype of function mk_wcwidth_cjk(), wcs_wcwidth_cjk(), and etc.
+// implemented by Markus Kuhn Ph.D., 2007.
+
...
+  int mid;
+#else
+// Fix types of struct internal's member and 'max' argument of function bisearch().
+// by Z.OOL. <zool@zool.jpn.org>
+
+struct interval {
+  wchar_t first;
+  wchar_t last;
...
+
+/* auxiliary function for binary search in interval table */
+static int bisearch(wchar_t ucs, const struct interval *table, size_t max) {
+  size_t min = 0;
+  size_t mid;
+#endif
+
...
@@ -109,10 +445,22 @@ utf8_width(wchar_t wc)
 {
 	int	width;
