...
index 40639af..78c98df 100644
--- a/options-table.c
+++ b/options-table.c
@@ -770,5 +770,28 @@ const struct options_table_entry options_table[] = {
 	  .default_num = 1
 	},
...
+#endif
 	...
 	setlocale(LC_TIME, "");
 	tzset();
@@ -351,6 +360,13 @@ main(int argc, char **argv)
...
index def0fee..94ba2e5 100644
--- a/tty-term.c
+++ b/tty-term.c
@@ -543,6 +543,15 @@ tty_term_find(char *name, int fd, char **cause)
...
+#else
 	...
 	memset(term->acs, 0, sizeof term->acs);
 	if (tty_term_has(term, TTYC_ACSC))
@@ -551,6 +560,7 @@ tty_term_find(char *name, int fd, char **cause)
...
@@ -109,10 +422,22 @@ utf8_width(wchar_t wc)
 {
 	int	width;
