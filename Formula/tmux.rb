...
index 5f2cac46..34684f6c 100644
--- a/input.c
+++ b/input.c
...
index d7c196e1..1477c4f6 100644
--- a/screen-write.c
+++ b/screen-write.c
@@ -992,6 +992,11 @@ screen_write_alignmenttest(struct screen_write_ctx *ctx)
 	memcpy(&gc, &grid_default_cell, sizeof gc);
 	utf8_set(&gc.data, 'E');
...
@@ -1026,6 +1031,11 @@ screen_write_insertcharacter(struct screen_write_ctx *ctx, u_int nx, u_int bg)
...
@@ -1054,6 +1064,11 @@ screen_write_deletecharacter(struct screen_write_ctx *ctx, u_int nx, u_int bg)
 	if (s->cx > screen_size_x(s) - 1)
 		return;
...
@@ -1082,6 +1097,11 @@ screen_write_clearcharacter(struct screen_write_ctx *ctx, u_int nx, u_int bg)
 	if (s->cx > screen_size_x(s) - 1)
 		return;
...
@@ -1100,9 +1120,18 @@ screen_write_insertline(struct screen_write_ctx *ctx, u_int ny, u_int bg)
...
@@ -1147,12 +1176,26 @@ screen_write_deleteline(struct screen_write_ctx *ctx, u_int ny, u_int bg)
 	struct grid	*gd = s->grid;
 	struct tty_ctx	 ttyctx;
...
@@ -1198,6 +1241,11 @@ screen_write_clearline(struct screen_write_ctx *ctx, u_int bg)
...
@@ -1227,6 +1275,11 @@ screen_write_clearendofline(struct screen_write_ctx *ctx, u_int bg)
...
@@ -1254,6 +1307,11 @@ screen_write_clearstartofline(struct screen_write_ctx *ctx, u_int bg)
 		return;
 	}
...
@@ -1301,6 +1359,11 @@ screen_write_reverseindex(struct screen_write_ctx *ctx, u_int bg)
...
@@ -1345,12 +1408,22 @@ screen_write_linefeed(struct screen_write_ctx *ctx, int wrapped, u_int bg)
 	struct grid		*gd = s->grid;
 	struct grid_line	*gl;
...
@@ -1358,7 +1431,17 @@ screen_write_linefeed(struct screen_write_ctx *ctx, int wrapped, u_int bg)
 	}
...
@@ -1383,6 +1466,11 @@ screen_write_scrollup(struct screen_write_ctx *ctx, u_int lines, u_int bg)
 		ctx->bg = bg;
 	}
...
+
 	for (i = 0; i < lines; i++) {
 		grid_view_scroll_region_up(gd, s->rupper, s->rlower, bg);
 		screen_write_collect_scroll(ctx, bg);
@@ -1407,6 +1495,11 @@ screen_write_scrolldown(struct screen_write_ctx *ctx, u_int lines, u_int bg)
...
@@ -1431,6 +1524,11 @@ screen_write_clearendofscreen(struct screen_write_ctx *ctx, u_int bg)
 	struct tty_ctx	 ttyctx;
...
@@ -1460,6 +1558,11 @@ screen_write_clearstartofscreen(struct screen_write_ctx *ctx, u_int bg)
 	struct tty_ctx	 ttyctx;
...
@@ -1483,6 +1586,11 @@ screen_write_clearscreen(struct screen_write_ctx *ctx, u_int bg)
 	struct tty_ctx	 ttyctx;
 	u_int		 sx = screen_size_x(s), sy = screen_size_y(s);
...
@@ -1751,6 +1859,11 @@ screen_write_collect_end(struct screen_write_ctx *ctx)
 		}
 	}
...
@@ -2138,6 +2251,61 @@ screen_write_rawstring(struct screen_write_ctx *ctx, u_char *str, u_int len,
...
index 2a79f3e3..d7d8b766 100644
--- a/server-fn.c
+++ b/server-fn.c
@@ -33,7 +33,14 @@ static void		 server_destroy_session_group(struct session *);
 void
...
index 76dd60dc..3043b5e8 100644
--- a/tmux.h
+++ b/tmux.h
...
+#endif
 	...
 	u_int		 oox;
 	u_int		 ooy;
@@ -2326,6 +2371,11 @@ void	tty_set_path(struct tty *, const char *);
...
@@ -2356,6 +2406,11 @@ void	tty_cmd_scrolldown(struct tty *, const struct tty_ctx *);
...
@@ -2938,6 +2993,10 @@ void	 screen_write_setselection(struct screen_write_ctx *, const char *,
 	     u_char *, u_int);
...
@@ -3355,6 +3414,26 @@ struct window_pane *spawn_pane(struct spawn_context *, char **);
 /* regsub.c */
